<script lang="ts">
    ...
    let timestamp = $derived(
        glucoseData?.timestamp ? new Date(glucoseData.timestamp) : null,
    );
    ...
    let valueExtent = $derived.by(() => {
        if (sortedData.length === 0)
            return [lowThreshold - 10, highThreshold + 10];
    ...
    let yScale = $derived((value: number) => {
        ...
        return range === 0
            ? graphHeight / 2
            : graphHeight - ((value - minValue) / range) * graphHeight;
    });
            ...
            >
                ...
                <g transform="translate({margin.left}, {margin.top})">
                    ...
                    <text
                        ...
                        y={lowRangeY + 4}
                        text-anchor="end"
                        class="text-xs fill-gray-500 font-semibold"
                        >{lowThreshold}</text
                    >
                    ...
                    <text
                        ...
                        y="15"
                        text-anchor="end"
                        class="text-2xl font-bold font-mono {glucoseColor}"
                        >{glucoseValue}</text
                    >
                    <text
                        x={graphWidth - 10}
                        y="30"
                        text-anchor="end"
                        class="text-xs fill-gray-500 font-medium">{unit}</text
                    >
                    <text
                        x={graphWidth - 10}
                        y="42"
                        text-anchor="end"
                        class="text-xs fill-gray-500">{timeAgo}</text
                    >
                </g>
                ...
                <text
                    ...
                    text-anchor="middle"
                    transform="rotate(-90, 12, {margin.top + graphHeight / 2})"
                    class="text-xs fill-gray-500">mg/dL</text
                >
            </svg>
        </div>
    {:else}
        <div
            class="flex-1 flex items-center justify-center text-gray-500 min-h-[120px]"
        >
