<script lang="ts">
    ...
    let brightnessPercent = $derived(Math.round((brightness / maxBrightness) * 100));
    ...
    $effect(() => {
        if (!initialized && brightnessPercent > 0) {
            ...
            if (diff > 5) { // Increased threshold to 5%
                console.log("Updating brightness from", localBrightnessPercent, "to", brightnessPercent);
                localBrightnessPercent = brightnessPercent;
            }
        } else if (userInteracting) {
            console.log("Blocking brightness update because user is interacting");
        }
    });
    ...
    async function setBrightness(brightnessPercent: number) {
        ...
        const newBrightness = Math.round((brightnessPercent / 100) * maxBrightness);
        
        isLoading = true;
        try {
            await DashboardAppService.SetLightBrightness(widget.id, newBrightness);
        } catch (error) {
            console.error("Failed to set brightness:", error);
            // Revert to server value on error
            localBrightnessPercent = Math.round((brightness / maxBrightness) * 100);
        } finally {
            isLoading = false;
        }
    }
...
>
    ...
    <div 
        class="flex items-center gap-3 mb-4 cursor-pointer hover:bg-gray-50 hover:bg-opacity-5 transition-colors duration-200"
    ...
    >
        <div
            class="w-10 h-10 border-2 transition-all duration-300 flex items-center justify-center rounded"
            class:bg-yellow-400={isOn && !isLoading}
            class:border-yellow-400={isOn && !isLoading}
        ...
        <div class="flex-1">
            <h3 class="text-lg md:text-xl font-semibold m-0 capitalize text-white">
                {entityName}
            </h3>
            ...
            {#if isOn && localBrightnessPercent > 0}
                <div class="text-xs text-gray-400 mt-1">
                    {localBrightnessPercent}% brightness
                </div>
            {/if}
        </div>
    </div>
    
    <!-- Brightness Slider (only show when light is on) -->
    {#if isOn && !isLoading}
        <div class="mt-2">
            <label for={`brightness-${widget.id}`} class="block text-sm text-gray-300 mb-2">Brightness</label>
            <input 
